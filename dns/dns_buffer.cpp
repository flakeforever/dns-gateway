...
void dns_buffer::write_domain(const std::string domain) {
  ...
  while (label_ptr == 0) {
    ...
    if (sub_name == "" || sub_name.length() > dns::max_label_size) {
      throw std::system_error(dns::errc::make_error_code(
          dns::errc::error_code::buffer_format_error));
    }
...
char *dns_buffer::data() const { return (char *)&buffer_[0]; }
} // namespace dns