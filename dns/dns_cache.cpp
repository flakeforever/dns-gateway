...
namespace dns
{
    ...
    void dns_cache_entry::set_ttl(uint32_t value)
    {
        ttl_ = value;
    }
    ...
    asio::awaitable<dns_cache_entry *> dns_cache::get_free_cache()
    {
        if (free_cache_entries_.empty())
        {
            ...
            for (const auto &entry : cache_entries_)
            {
                ...
                double weight = static_cast<double>(cache_entry->access_count_) / (elapsed_time + 1);
    ...
    asio::awaitable<void> dns_cache::add_cache(const std::string &domain, uint8_t type, dns_cache_entry *cache_entry)
    {
    ...
    asio::awaitable<dns_cache_entry *> dns_cache::query_cache(const std::string &domain, uint8_t type)
    {
        std::string key = generate_key(domain, type);
