...
namespace dns {
dns_question::dns_question(std::string q_name, uint16_t q_type,
                           uint16_t q_class) {
  q_name_ = q_name;
  q_type_ = q_type;
  q_class_ = q_class;
...
dns_answer::dns_answer(std::string a_name, dns::anwser_type a_type,
                       uint16_t a_class, uint32_t a_ttl) {
  a_name_ = a_name;
...
dns_aaaa_answer::dns_aaaa_answer(std::string a_name, uint16_t a_class,
                                 uint32_t a_ttl)
    : dns_answer(a_name, dns::anwser_type::aaaa, a_class, a_ttl) {
  memset(addr, 0, sizeof(addr));
}

dns_cname_answer::dns_cname_answer(std::string a_name, uint16_t a_class,
                                   uint32_t a_ttl)
    : dns_answer(a_name, dns::anwser_type::cname, a_class, a_ttl) {
  domain = "";
}

dns_ns_answer::dns_ns_answer(std::string a_name, uint16_t a_class,
                             uint32_t a_ttl)
    : dns_answer(a_name, dns::anwser_type::ns, a_class, a_ttl) {
  domain = "";
}

dns_txt_answer::dns_txt_answer(std::string a_name, uint16_t a_class,
                               uint32_t a_ttl)
    : dns_answer(a_name, dns::anwser_type::txt, a_class, a_ttl) {
  text = "";
}

dns_mx_answer::dns_mx_answer(std::string a_name, uint16_t a_class,
                             uint32_t a_ttl)
...
void dns_package::add_question(std::string domain,
                               dns::anwser_type question_type) {
  ...
  std::shared_ptr<dns_question> question = std::make_shared<dns_question>(
      domain, static_cast<uint16_t>(question_type),
      static_cast<uint16_t>(dns::class_type::in));
  questions_.push_back(question);
}

void dns_package::add_anwser(std::string domain, dns::anwser_type anwser_type,
                             std::string value) {
  ans_count_++;
  ...
  switch (anwser_type) {
  case anwser_type::a: {
    std::shared_ptr<dns_a_answer> answer = std::make_shared<dns_a_answer>(
        domain, static_cast<uint16_t>(dns::class_type::in), dns::default_ttl);
  ...
  case anwser_type::aaaa: {
    std::shared_ptr<dns_aaaa_answer> answer = std::make_shared<dns_aaaa_answer>(
        domain, static_cast<uint16_t>(dns::class_type::in), dns::default_ttl);
  ...
  case anwser_type::cname: {
    std::shared_ptr<dns_cname_answer> answer =
        std::make_shared<dns_cname_answer>(
            domain, static_cast<uint16_t>(dns::class_type::in),
            dns::default_ttl);
  ...
  case anwser_type::txt: {
    std::shared_ptr<dns_txt_answer> answer = std::make_shared<dns_txt_answer>(
        domain, static_cast<uint16_t>(dns::class_type::in), dns::default_ttl);
...
bool dns_package::parse(const char *data, uint16_t data_length) {
  ...
  for (int i = 0; i < que_count_; ++i) {
    ...
    std::shared_ptr<dns_question> question =
        std::make_shared<dns_question>(q_domain, q_type, q_class);
    questions_.push_back(question);
  }
  ...
  for (int i = 0; i < ans_count_; ++i) {
    ...
    case anwser_type::a: {
      std::shared_ptr<dns_a_answer> answer =
          std::make_shared<dns_a_answer>(a_domain, a_class, a_ttl);
    ...
    case anwser_type::aaaa: {
      std::shared_ptr<dns_aaaa_answer> answer =
          std::make_shared<dns_aaaa_answer>(a_domain, a_class, a_ttl);
      buffer.read_buffer((char *)&answer->addr[0], a_length);
      answers_.push_back(answer);
      break;
    }
    case anwser_type::cname: {
      std::shared_ptr<dns_cname_answer> answer =
          std::make_shared<dns_cname_answer>(a_domain, a_class, a_ttl);
    ...
    case anwser_type::ns: {
      std::shared_ptr<dns_ns_answer> answer =
          std::make_shared<dns_ns_answer>(a_domain, a_class, a_ttl);
      answer->domain = buffer.read_domain();
      if (answer->domain == "") {
        return false;
      }
    ...
    case anwser_type::mx: {
      std::shared_ptr<dns_mx_answer> answer =
          std::make_shared<dns_mx_answer>(a_domain, a_class, a_ttl);
    ...
    case anwser_type::txt: {
      std::shared_ptr<dns_txt_answer> answer =
          std::make_shared<dns_txt_answer>(a_domain, a_class, a_ttl);
...
void dns_package::output() {
  common::log.debug("++++ DNS Package +++++");
  common::log.debug("ID: %d", id_);
  common::log.debug("FLAG: %d", flags_);
  common::log.debug("\t %s", qr_to_string(flag_qr()).c_str());
  common::log.debug("\t %s", opcode_to_string(flag_opcode()).c_str());
  common::log.debug("\t %s", rcodes_to_string(flag_rcode()).c_str());
  common::log.debug("\t");
  common::log.debug("\t");
  common::log.debug("\t");
  common::log.debug("Question Count: %d", que_count_);
  common::log.debug("Answer Count: %d", ans_count_);
  common::log.debug("Auth Count: %d", aut_count_);
  common::log.debug("Additional: %d", add_count_);

  for (std::shared_ptr<dns_question> q : questions_) {
    common::log.debug("Question => Name(%s), Type(%s), Class(%s)",
                 q->q_name().c_str(), rtypes_to_string(q->q_type()).c_str(),
                 classes_to_string(q->q_class()).c_str());
  }

  for (std::shared_ptr<dns_answer> a : answers_) {
    common::log.debug("Answer => Name(%s), Type(%s), Class(%s), TTL(%d), ",
                 a->a_name().c_str(), rtypes_to_string(a->a_type()).c_str(),
                 classes_to_string(a->a_class()).c_str(), a->a_ttl());

    switch (static_cast<anwser_type>(a->a_type())) {
    case anwser_type::a: {
      std::shared_ptr<dns_a_answer> answer =
          std::dynamic_pointer_cast<dns_a_answer>(a);
      common::log.debug("RData(%s)",
                   ipv4_to_string(answer->addr, sizeof(answer->addr)).c_str());
      break;
    }
    case anwser_type::aaaa: {
      std::shared_ptr<dns_aaaa_answer> answer =
          std::dynamic_pointer_cast<dns_aaaa_answer>(a);
      common::log.debug("RData(%s)",
                   ipv6_to_string(answer->addr, sizeof(answer->addr)).c_str());
      break;
    }
    case anwser_type::cname: {
      std::shared_ptr<dns_cname_answer> answer =
          std::dynamic_pointer_cast<dns_cname_answer>(a);
      common::log.debug("RData(%s)", answer->domain.c_str());
      break;
    }
    case anwser_type::ns: {
      std::shared_ptr<dns_ns_answer> answer =
          std::dynamic_pointer_cast<dns_ns_answer>(a);
      common::log.debug("RData(%s)", answer->domain.c_str());
      break;
    }
    case anwser_type::mx: {
      std::shared_ptr<dns_mx_answer> answer =
          std::dynamic_pointer_cast<dns_mx_answer>(a);
      common::log.debug("RData(%s %d)", answer->domain.c_str(), answer->priority);
      break;
    }
    case anwser_type::txt: {
      std::shared_ptr<dns_txt_answer> answer =
          std::dynamic_pointer_cast<dns_txt_answer>(a);
      common::log.debug("RData(%s %d)", answer->text.c_str());
      break;
    }
    default:
      break;
...
uint8_t dns_package::flag_qr() { return (flags_ & 0x8000) >> 15; }

uint8_t dns_package::flag_opcode() { return (flags_ & 0x7800) >> 11; }

uint8_t dns_package::flag_rcode() { return (flags_ & 0x000F); }

uint8_t dns_package::flag_ad() { return (flags_ >> 5) & 0x01; }

uint8_t dns_package::flag_ra() { return (flags_ >> 7) & 0x01; }

void dns_package::flag_qr(uint8_t qr) { flags_ |= (qr & 0x01) << 15; }
...
void dns_package::flag_rcode(uint8_t rcode) { flags_ |= (rcode & 0x0F) << 0; }
...
bool dns_package::string_to_ipv4(const std::string &address, uint8_t *buffer,
                                 std::size_t size) {
  if (size < 4) {
    return false;
  }
...
bool dns_package::string_to_ipv6(const std::string &address, uint8_t *buffer,
                                 std::size_t size) {
  if (size < 16) {
    return false;
  }
