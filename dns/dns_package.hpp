...
enum class qr_type { request = 0, response = 1 };
...
class dns_question : public dns_question_property,
                     public std::enable_shared_from_this<dns_question> {
public:
...
class dns_answer : public dns_answer_property,
                   public std::enable_shared_from_this<dns_answer> {
public:
  dns_answer(std::string a_name, dns::anwser_type a_type, uint16_t a_class,
             uint32_t a_ttl);
};
...
class dns_package : public dns_packag_property,
                    public std::enable_shared_from_this<dns_package> {
public:
  dns_package();

  void add_question(std::string domain, dns::anwser_type question_type);
  void add_anwser(std::string domain, dns::anwser_type anwser_type,
                  std::string value);
...
protected:
  ...
  bool string_to_ipv4(const std::string &address, uint8_t *buffer,
                      std::size_t size);
  bool string_to_ipv6(const std::string &address, uint8_t *buffer,
                      std::size_t size);
