...
namespace socks
{
    ...
    bool socks_buffer::write_address(socks::address_type address_type, const std::string &address, uint16_t port)
    {
    ...
    bool socks_buffer::read_address(socks::address_type address_type, std::string &address, uint16_t &port)
    {
        if (address_type == socks::address_type::ipv4)
        {
            char ipv4_buffer[4];
    ...
    char *socks_buffer::data() const
    {
        return data_;
    }

    size_t socks_buffer::size() const
    {
        return size_;
    }

    size_t socks_buffer::position() const
    {
        return position_;
    }
